...
{    
	...
	printk(KERN_INFO "%s %s writing %d \n",    
	   SS_NAME, VERSION, (at_table->at[0]).object_type);
    return len;    
}  

/*check authority*/
static int getSbjType(uid_t uid){
	int i;
...
static int getObjType(char* filename){
	int i;
...
static int enquireAT(int sbj_type,int obj_type, char* bit, int len){
	int i;
	//char bit[14];
...
static int enquireSS(uid_t uid, char* filename, char* at_bit, int len){
...
{    
     ...
     * functions    
     */    
	sbj_file = create_proc_entry("SubjectTable", 0644, ss_dir);    
	obj_file = create_proc_entry("ObjectTable", 0644, ss_dir);    
    at_file = create_proc_entry("AuthorityTable", 0644, ss_dir);    
